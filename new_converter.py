    ...
    for measure in track.measures:
        ...
        for voice in measure.voices:
            for beat in voice.beats:
                file.write("%s" % 'b ' + str(beat.duration.value) + ' ' + str(int(beat.duration.isDotted)) + str(beat.duration.tuplet.times) + str(beat.duration.tuplet.enters) + ' ' + str(beat.status.value) + '\n')
    ...
    for i, line in enumerate(file):
        if line[:2] == 'b ':      # Beat
            ...
            try:
                ...
                tuplet_times = int(line[parser+2 : parser+3])
                tuplet_enters = int(line[parser+3 : line.find(' ', parser+3)])
